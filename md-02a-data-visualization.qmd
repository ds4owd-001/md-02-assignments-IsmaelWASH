...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    ```{r}
    ggplot(data=gapminder_2007,mapping=aes(x=continent,y=lifeExp,fill=continent))+
      geom_boxplot(outlier.shape = NA) #credit: https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(data=gapminder_summary_2007,mapping=aes(x=continent,y=count))+
      geom_col()
    ```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: *the plot is identical because gapminder_2007 gives the life expectancy (median value) for each country in 2007. gapminder_summary_2007 summarizes the gapminder_2007 table, grouping countries by continent and counting the number of observations per continent (the number of countries). Therefore we use geom_col with gapminder_summary_2007 because we already have the* *values*. *Whereas using geom_bar() with gapminder_2007 data and specifying the continent on the x-axis, the function will calculate the observations according to the data*

    ```{r}
    ggplot(data=gapminder_2007,mapping=aes(x=continent))+
      geom_bar()
    ```
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    *geom_histogramm() is similar to geom_bar() in that it allows you to create bar charts. Both count the number of observations for each variable on the x-axis and present it as a bar.*

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:*By adding the argument breaks, we specify the different classes of life expectancy (x-axis) for which we wish to know the associated number (count). seq* *() function arguments: start number, end number and jump between classes. Changing these numbers has an impact on the histogram. A jump of 10 will reduce the number of classes compared with a jump of 4.*

    ```{r}
    ggplot(data=gapminder_2007,mapping=aes(x=lifeExp,fill=continent))+
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    *In* `country_colors` *we have predefined colors for every country in the world.*

    *With* `?country_colors` *we have more precision, its color schemes for the countries and continents in the Gapminder data*
